...
```{r}
x <- seq(0,10,.1)
d <- tibble(rate=seq(0.1,1.1,.2)) %>%
    mutate(dist=map(rate, ~ data.frame(x=x,y=dexp(x, rate=.)))) %>% 
    unnest()

d
d %>% ggplot(aes(x=x,y=y, group=rate, color=as.factor(rate))) +
    geom_line()
```
Plot some gamma distributions:
```{r}
x <- seq(0,10,.1)
d <- tibble(shape=seq(1,10,2)) %>%
    mutate(dist=map(shape, ~ data.frame(x=x,
                                        y=dgamma(x, shape=., rate=1)))) %>% 
    unnest()

d
d %>% ggplot(aes(x=x,y=y, group=shape, color=as.factor(shape))) +
    geom_line()
```

...
I would have thought that the pred or both models would have been notable better than the intervept only model.
...
```{r, fig.width=10}
df %>%
    ggplot(aes(x=district_id, y=mu, ymin=hdpi.low, ymax=hdpi.high, fill=model, color=model, shape=model)) +
    geom_point(size=3)
    
```

would be nice to add sample size to this...

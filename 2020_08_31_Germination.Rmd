...
```
There is no inverse logit because it is built in to the berouli function
...
model{
    ...
    for ( i in 1:N ) 
        if ( germ[i] == 0 ) target += log_mix(p[i], bernoulli_lpmf(1 | p[i]), bernoulli_lpmf(0 | p[i]) + exponential_lccdf(day[i] | lambda[i]));
    for ( i in 1:N ) 
        if ( germ[i] == 1 ) target += bernoulli_lpmf(0 | p[i]) +  exponential_lpdf(day[i] | lambda[i]);
    for ( i in 1:N ) 
        if ( germ[i] == 1 ) day[i] ~ exponential( lambda[i] );
}
"
m1.5alt <- stan(model_code=stanmodel1.5alt, data=d)
```

```{r}
precis(m1.5alt, depth = 2)
```

```{r}
post <- as.data.frame(m1.5alt)
head(post)
```
...
## allow both shape and rate of ZI gamma to vary

Need to think about link functions...
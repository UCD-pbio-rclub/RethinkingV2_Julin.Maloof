...
dat_list <- list( admit = d$admit,
    ...
    gid = ifelse( d$applicant.gender=="male" , 1L , 2L ),
...
m14M3 <- ulam(
  alist(
    admit ~ dbinom( applications , p ), 
    logit(p) <- a_bar + z_a[gid]*sigma_a + z_delta[dept_id]*sigma_d,
    a_bar ~ dnorm(0,1.5),
    z_a[gid] ~ dnorm( 0 , 1.5 ), 
    z_delta[dept_id] ~ dnorm( 0 , 1.5 ),
    sigma_a ~ dexp(1),
    sigma_d ~ dexp(1)),
  data=dat_list , 
  chains=4 , 
...
non-centred is slower to fit and has more divergent transitions but also has more effective samples and slightly fewer parameters.
...
mpdf1.3 <- ulam(alist(
  use ~ dbinom(1, p),
  logit(p) <- alphabeta[1, d_id] + alphabeta[2, d_id]*urban,
  transpars> matrix[2, d_id]:alphabeta <- compose_noncentered(sigma, L_Rho, z),
  matrix[d_id, 2]:z ~ normal(0,1.5),
  vector[2]:sigma ~ lkj_corr_cholesky( 2 ),
  cholesky_factor_corr[2]: L_Rho ~ lkj_corr(2),

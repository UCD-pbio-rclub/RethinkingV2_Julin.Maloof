...
## 15M2
_In earlier chapters, we threw away cases from the primate milk data, so we could use the neocortex variable. Now repeat the WAIC model comparison example from Chapter 6, but use imputation on the neocortex variable so that you can include all of the cases in the original data. The simplest form of imputation is acceptable. How are the model comparison results affected by being able to include all of the cases?_

The models with complete cases:
```{r}
## R code 5.27
data(milk)
d <- milk
...
d2 <- milk
d2$K <- scale( d$kcal.per.g )
d2$N <- scale( d$neocortex.perc )
d2$M <- scale( log(d$mass) )

## R code 5.31
dcc <- d[ complete.cases(d$K,d$N,d$M) ,  c("K","N","M")]
dcc2 <- d2[ complete.cases(d2$K,d2$N,d2$M) ,  c("K","N","M")]
...
m5.x <- ulam( alist(
        ...
        a ~ dnorm( 0 , 0.2 ) ,
        sigma ~ dexp( 1 )
    ) , data=dcc, log_lik = TRUE, chains = 4, cores = 4 )
```
...
m5.5 <- ulam(
    alist(
        ...
        bN ~ dnorm( 0 , 0.5 ) ,
        sigma ~ dexp( 1 )
    ) , data=dcc2, log_lik = TRUE, chains = 4, cores = 4 )
```
...
m5.x <- ulam( alist(
        ...
        a ~ dnorm( 0 , 0.2 ) ,
        sigma ~ dexp( 1 )
    ) , data=dall, log_lik = TRUE, chains = 4, cores = 4 )
...
```{r}
pred <- link(mh1.1, data=list(A=age))
pred.plot <- tibble(
  A=age,
  M=apply(pred,2,mean),
  lower=apply(pred,2,HPDI)[1,],
  upper=apply(pred,2,HPDI)[2,],
)
```
